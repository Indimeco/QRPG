...
function loadCharacterSheetSource()
    if characterSheetSource == nil then
        ...
        if characterSheetSource ~= nil then
            ...
            characterSheetSource.setLock(true)
...
function unloadCharacterSheetSource()
    if characterSheetSource ~= nil then
        ...
        characterSheetSource.setLock(false)
...
function loadCharacter(arg)
    ...
    if characterSheetSource.getVar('onJournalLoad') ~= nil then
        characterSheetSource.call('onJournalLoad',{color}) -- callback function for loaded character journal
    end
end
    ...
    local function createIncrementButtonsFromTable(table)
        for key,object in pairs(table) do
            if type(object) == 'table' then
                ...
                Global.call('addButton',{object.getGUID(),modSize,modSize,modSize,'-','modifierSub',self,lookupButtonData[key]['1stPos'],{180,0,0}})
                Global.call('addButton',{object.getGUID(),modSize,modSize,modSize,'+','modifierAdd',self,lookupButtonData[key]['2ndPos'],{180,0,0}})
            end
        end
    end
    ...
    local function createToggleButtonsFromTable(table)
        for key,object in pairs(table) do
            if type(object) == 'table' then
                ...
                local modSize = lookupButtonData[key]['modSize']
                Global.call('addButton',{object.getGUID(),modSize,modSize,modSize,' ','modifierToggle',self,lookupButtonData[key]['pos'],{180,0,0}})
            end
        end
    end
        ...
        for key,object in pairs(table) do
            if type(object) == 'table' then
                ...
                for i=1,lookupButtonData[key]['listSize'] do
                    ...
                    Global.call('addButton',{object.getGUID(),modSize,modSize,modSize,'x','lookupButtonData'..key..i,self,pos,{180,0,0}})
                end
            end
        end
    end
    ...
    local function createTabsButtonsFromTable(table)
        for key,object in pairs(table) do
            if type(object) == 'table' then
                ...
                local modSize = lookupButtonData[key]['modSize']
                Global.call('addButton',{object.getGUID(),modSize,modSize,modSize,' ','modifierTabs',self,lookupButtonData[key]['pos'],{180,0,0}})
            end
        end
    end
...
function closeBook(color) -- Destroys open book model, creates new closed book model which holds character data from the character sheet
    ...
    if characterSheetSource.getVar('onJournalClosed') ~= nil then
        characterSheetSource.call('onJournalClosed',{color,obj}) -- callback function for an exiting journal
    end
end
...
script = script..'\n\n--------------------------- Character Information ---------------------------\n'
    ...
    for key,value in pairs(objectOffsetList) do
        ...
        elseif value['name'] == 'Inventory' then
            script = script..'\n'..key..' = \[\['..getObjectFromGUID("TOOLS").call('callGetFullText',{color})..'\]\]'
        else
