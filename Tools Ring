...
layoutExceptions = {
    ...
    '627e7a', -- Desk Transport Chestsss
    'f99d56' -- Char Sheet Altar
}
...
function onload(save_state)
    ...
    invControllerButton = {
        ['Red']    = getObjectFromGUID('1ed590'),
        ['Orange'] = getObjectFromGUID('4ccf5a'),
        ['Yellow'] = getObjectFromGUID('af28ab'),
        ['Green']  = getObjectFromGUID('a94dbc'),
        ['Blue']   = getObjectFromGUID('9e1d31'),
        ['Purple'] = getObjectFromGUID('53ecd4')
    }
    ...
    invControllerInvCard['Master'].setVar('qrpgColor', 'Master')
    invControllerInvCard['initiative'].setVar('qrpgColor','initiative')
    ...
    Global.call('addButton', {monsterPlatform[2].getGUID(),2100,380,190,'Update Figurine','forceUpdateMini',self,{0,0.35,-4.2},{0,180,0}})
    ...
    Global.call('addButton', {monsterPlatform[1].getGUID(),2100,380,190,'Update Figurine','forceUpdateMini',self,{0,0.35,-4.2},{0,180,0}})
...
function loadPlayer(param)
    ...
    invControllerButton[color].setVar('qrpgColor',color)
    Global.call('addButton', {invControllerButton[color].guid,500,450,120,color,'selectPlayer',self,{0,0.53,0},{0,90,0}})
    invControllerInvCard[color].setVar('qrpgColor', color)
...
function showHelp()
    if helpCard.getPosition()[2] > 100 then
        ...
        largeTablet.setValue('https://docs.google.com/document/d/1AhC_asYjGpv5cbHYm52QMHt6C24rBcDUkjK6mhOjEfM/edit?usp=sharing') -- tutorial document
    else
...
-- This function is called from a button - ergo obj is the button's object that made the call
    colorSelector = obj.getVar('qrpgColor')
...
-- This function is called from a button - ergo obj is the button that made the call.
    local color = obj.getVar('qrpgColor')
...
function toggleStatCounters(obj) -- Hides the second set of counters under the table.
    ...
    if statCountersToggled[platNumber] == 0 then
        for i=7,12 do
            ...
            monsterPlatformValueCounters[platNumber][i].setLock(true)
...
function toggleSecondPlatform() -- Hides the second platform under the table.
    ...
    if secondPlatformToggled == 0 then
        monsterPlatform[2].setLock(true)
        ...
        monsterPlatformDescCard[2].setLock(true)
        monsterPlatformDescCard[2].setPosition({pos[1],pos[2]-35,pos[3]})
        for i=1,13 do
            monsterPlatformValueCounters[2][i].setLock(true)
...
function showWelcome() -- Used to prepare the table for publication with the welcome card
  currentLayoutGet(1)
  currentLayout = 0
  startCard = spawnObject({type="Notecard"})
  startCard.setLock(true)
  startCard.setPosition({-74.29, 9.25, -0.03})
  startCard.setRotation({326.71, 90.00, 0.00})
  startCard.setScale({2.25, 2.25, 2.25})

  startCard.setName('Welcome to QRPG')
  startCard.setDescription('Thank you for joining the Beta of Quality RPG!\n[i]Please[/i] report any bugs and feel free to share your ideas, suggestions and experiences on Discord or on the workshop.\n\nPress '..string.char(9432)..' on the dashboard (below) for links\n\n~QRPG Devs\n[i]rolandostar[/i] & [i]Indimeco[/i]')

  startCard.setLuaScript([[
    function onload()
      Global.call('addButton',{self.getGUID(),140,140,80,'Go!','done',self,{-0.3,4.5,-0.35},{0,180,0}})
    end
